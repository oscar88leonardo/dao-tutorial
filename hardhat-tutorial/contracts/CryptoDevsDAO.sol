...
contract CryptoDevsDAO is Ownable {
    ...
    // initialize variables for  contracts we are importing
    ...
    // by someone who owns at least 1 CryptoDevsNFT  
    modifier nftHolderOnly() {
        require(cryptoDevsNFT.balanceOf(msg.sender) > 0, "NOT_A_DAO MEMBER");
        _;
    }
    ...
    // @param vote - the type of vote the want to cast
    ...
    function executeProposal(uint256 proposalIndex)
        ...
        inactiveProposalOnly(proposalIndex)
        {
            ...
            // if the proposal has more YAY votes than NAY votes purchase the NFT from the Fakemarketplace
            if(proposal.yayVotes > proposal.nayVotes) {
                uint256 nftPrice = nftMarketplace.getPrice();
                require(address(this).balance >= nftPrice, "NOT_ENOUGHS_FUNDS");
